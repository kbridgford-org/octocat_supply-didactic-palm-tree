...
2. Navigate to the [organization's Linked artifacts page](https://github.com/orgs/octodemo/artifacts).
...
6. Apply the two filters `runtime-risk:internet-exposed` and `severity:high` (optionally, if there are too many results in your demo organization, add a filter for the repository name `repo:octodemo/`).
7. You should see the alert `SQL query built from user-controlled sources` in the list. Explain how the previously seen artifact metadata allowed for a better prioritization of this alert, thanks to the `internet-exposed` runtime risk.
    1. (optional): Use this direct link to the filtered view: <https://github.com/orgs/octodemo/security/alerts/code-scanning?query=is%3Aclosed+runtime-risk%3Ainternet-exposed+severity%3Ahigh++repo%3A>
